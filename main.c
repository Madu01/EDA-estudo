...
//Montando lista das Industrias:
    ...
    parq = fopen("/home/alunos/Documentos/EDA-Duda/Industria.txt", "r");
    if(parq==NULL)
        return NULL;
...
TNoC *CriaListaComercio()
{
    FILE* parqc;
    TCadastro pregauxc;
    TNoC *plistac, *pnovonoc, *pAuxC, *fim;
//Montando lista dos Comercios:
    
    parqc = fopen("/home/alunos/Documentos/EDA-Duda/Comercio.txt", "r");
    if(parqc==NULL)
        return NULL;
    
    plistac = NULL;
    while(fscanf(parqc,"%s %s %s %s", pregauxc.CNPJ, pregauxc.RazaoSocial, pregauxc.Cidade, pregauxc.Fone)>0) {
        pnovonoc = (TNoC *)malloc(sizeof(TNoC));
        pnovonoc->Dados = (TCadastro *)malloc(sizeof(TCadastro));
        ...
        pAuxC = pnovsonoc;
        fim->Prox = pnovonoc->Dados;
        pnovonoc->Ant = pAuxC;
        
        
        pnovonoc->Ant = NULL;
        pnovonoc->Prox = plistac;
        plistac = pnovonoc;
    }
...
void ImprimeComercio(TNoC *LCom){
    TNoC *plistac = LCom;
    printf("                     LISTA DE COMERCIO\n");
    printf("CNPJ            Razao Social                Cidade      Telefone\n");
    while(plistac != NULL) {
        printf("%s\t%s\t%s\t%s\n", plistac->Dados->CNPJ, plistac->Dados->RazaoSocial, plistac->Dados->Cidade, plistac->Dados->Fone);
        plistac = plistac->Prox;
    }
...
int CriaListaServico(TDescritorS *pLSer)
{
    FILE* parqc;
    TCadastro pregauxc;
    TNoC *plistac, *pnovonoc, *pauxc;
//Montando lista dos Servicos:
